...
FROM employees; -- 커미션이 null이면 0으로 치환

-- ALIAS : 별칭
SELECT 
first_name || ' ' || last_name 이름,
phone_number as 전화번호,
salary "급 여" -- 공백, 특수문자 포함된 별칭은 "" 묶는다
FROM employees;
...
-- 논리연산자
-- 급여가 14000이하 이거나 17000이상인 사원
SELECT first_name, salary
FROM employees
WHERE salary <= 10000 OR
salary >= 17000;

-- 급여가 4000이상, 17000이하인 사원의 목록
SELECT first_name, salary
FROM employees
...
-- 츌력 : 부서번호, 급여, 이름
-- 정렬 : 1차정렬 부서번호 오름차순 / 2차 정렬 급여 내림차순
SELECT department_id,
salary, first_name
FROM employees
ORDER BY department_id, -- 1차정렬
salary DESC; -- 2차정렬
...
POWER(2, 4), -- 제곱: 2의 4제곱
ROUND(3.5), -- 소숫점 반올림
ROUND(3.14159, 3), -- 소숫점 3자리까지 반올림으로 표현
TRUNC(3.5), --소숫점 버림
TRUNC(3.14159, 3), --소숫점 3자리까지 버림으로 표현
SIGN(-10) --부호 혹은 0
FROM dual;
...
SELECT SYSDATE FROM employees;  -- employees 레코드 개수만큼

-- 날짜 관련 단일행 함수
SELECT sysdate,
ADD_MONTHS(sysdate, 2), --2개월후
LAST_DAY(sysdate), -- 이번달 마지막날
MONTHS_BETWEEN( sysdate, '99/12/31'), --1999년 마지막날 이후 몇달이 지났는가?
NEXT_DAY(sysdate, 7),
ROUND(sysdate, 'MONTH'),
ROUND(sysdate, 'YEAR'),
TRUNC(sysdate, 'MONTH'),
...
-- TO NUMBER(s, fmt) : 문자열을 포맷에 맞게 수치형으로 변환
-- TO DATE(s, fmt) : 문자열을 포맷에 맞게 날짜형으로 변환
-- TO CHAR(o, fmt) : 숫자 or 날짜를 포맷에 맞게 문자형으로 변환
...
SELECT first_name, department_id,
DECODE 
(department_id, BETWEEN 10 AND 30, 'A-GROUP', 
BETWEEN 40 AND 50, 'B-GROUP', 
BETWEEN 60 AND 100, 'C-GROUP', 
'REMAINDER') 
 team
 FROM employees;
